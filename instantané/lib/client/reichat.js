...
function init() {
	...
	standby = function () {
		...
		var sendChat = function () {
			...
			if (/^\/[a-z0-9\-]+/.test(value) === true) {
				if (value === '/clear') {
					...
					util.addChatLog(Date.now(), 'PaintChatBot', 'commande invalide.');
				}
				return;
			}
		...
		view.chatInput = flagrate.createTextInput({
			placeholder: '(' + 'Dites quelque chose'.__() + ')',
			attribute: {
				maxlength: 128
			}
		...
		util.addChatLog(Date.now(), 'PaintChatBot', 'chargement'.__());
	...
	ready = function () {
		...
		util.addChatLog(Date.now(), 'PaintChatBot', 'prêt'.__());
	};
	...
	listener.socketOnChat = function (chat) {
		...
		if (chat.client) {
			...
			util.addChatLog(chat.time, 'PaintChatBot', chat.message.__());
		}
	};
	...
	ui.promptClientName = function (done, cancel) {
		...
		flagrate.createModal({
			...
			buttons: [
				{
					color: '@pink',
					label: client.name === '' ? 'Rejoint'.__() : 'OK',
	...
	ui.clearCanvas = function () {
		...
		flagrate.createModal({
			...
			text: 'Vous êtes sur de vouloir effacer canvas?'.__(),
			buttons: [
				{
					label: 'Yes'.__(),
					color: '@red',
